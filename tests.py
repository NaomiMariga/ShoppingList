...
class Tests(unittest.TestCase):
    ...
    def test_all_functions_returns_dictionary(self):
        ...
        self.assertIsInstance(self.shoppingList.edit_item(None, None, None, None, None, None, None, None), dict, "edit_item" + message)
        self.assertIsInstance(self.shoppingList.delete_item(None, None, None, None), dict, "delete_item" + message)

    def test_registration_accepts_valid_variables(self):
    ...
    def test_login_accepts_valid_variables(self):
    ...
    def test_login_accepts_registered_credentials_only(self):
        ...
        self.token = result3["message"]["token"]
        self.user_id = result3["message"]["user_id"]
        self.user_name = result3["message"]["user_name"]

    def test_if_provided_token_and_user_id_are_working(self):
    ...
    def test_shopping_list_addition(self):
        ...
        result3 = self.shoppingList.add_list(None, self.user_id, self.token)
        ...
        self.assertFalse(result3["success"], "invalid list_name given")
    ...
    def test_read_shopping_list(self):
        ...
        self.list = result1["message"][0]
        self.assertIsInstance(self.list, dict, "expected at least one item return as a dictionary")

    def test_edit_shopping_list(self):
        result1 = self.shoppingList.update_list(self.list["list_id"], self.token, self.user_id, "new_name")
    ...
    def test_add_items_to_the_list(self):
        result1 = self.shoppingList.add_item("item_name", "2", "", "400", self.list["list_id"], self.token, self.user_id)
        result2 = self.shoppingList.add_item("item_name", "", "kg", "", None, self.token, self.user_id)
        result3 = self.shoppingList.add_item(None, "", "", "", self.list["list_id"], self.token, self.user_id)
    ...
    def test_read_items(self):
        result1 = self.shoppingList.read_items(self.user_id, self.token, self.list["list_id"])
        ...
        self.items_list = result1["message"][0]
    ...
    def test_edit_item(self):
        result1 = self.shoppingList.edit_item(
            self.items_list["item_id"], self.list["list_id"], self.user_id, self.token, "new_item_name", "", "", "")
        result2 = self.shoppingList.edit_item(
    ...
    def test_delete_item(self):
        result1 = self.shoppingList.delete_item(self.items_list["item_id"], self.user_id, self.token, self.list["list_id"])
    ...
    def test_delete_shopping_list(self):
        result1 = self.shoppingList.delete_list(self.list["list_id"], self.user_id, self.token)
